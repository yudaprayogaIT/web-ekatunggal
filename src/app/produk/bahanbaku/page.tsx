...
export default async function AllBahanBakuPage() {
  ...
  if (!API_BASE) {
    throw new Error(
      "NEXT_PUBLIC_API_BASE_URL tidak terdefinisi. Silakan atur di Vercel."
    );
  }

  if (!TOKEN) {
    throw new Error("ERP_TOKEN tidak terdefinisi. Silakan atur di Vercel.");
  }
  ...
  // 4) Kumpulkan slug unik untuk kategori bahan baku
  const bahanBakuSet = new Set<string>();
  semuaProduk.forEach((p) => {
    const slug = p.kategori.toLowerCase().replace(/\s+/g, "-");
    if (!kategoriBarangJadiList.includes(slug)) {
