...
export default function Sidebar({
  ...
  // Auto‐scroll saat membuka sebuah kategori
  useEffect(() => {
    if (!openCatId) return;
    const container = containerRef.current;
    const itemEl = document.getElementById(openCatId);
  ...
  return (
    <section className="relative">
      ...
      <nav
        ref={containerRef}
        className="w-39 sm:w-55 lg:w-60 2xl:w-64 pr-4 max-h-[400px] lg:max-h-[480px] overflow-y-auto relative"
      >
        {sortedData.map((type) => {
          const isTypeOpen = openTypeId === type.id;
          return (
            <div key={type.id} className="mb-1 2xl:mb-4">
              {/* Type Header */}
              <button
                onClick={() => {
                  const nextType = isTypeOpen ? null : type.id;
                  setOpenTypeId(nextType);
                  setOpenCatId(null);
                  onSelectType(type.id);
                }}
                className="flex items-center space-x-2 font-bold cursor-pointer"
                    ...
                    {type.categories.map((cat) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            onClick={() => {
                              const nextCat = isCatOpen ? null : cat._id;
                              setOpenCatId(nextCat);
                              onSelectCategory(cat);
                            }}
                            className="flex items-center w-full text-xs sm:text-base space-x-1 sm:space-x-2 font-semibold cursor-pointer"
                          >
