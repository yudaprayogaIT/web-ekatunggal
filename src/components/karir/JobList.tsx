      ...
      <div className="bg-white border border-gray-100 rounded-xl md:rounded-4xl p-6 shadow-lg">
        ...
        <div className="overflow-y-auto max-h-79 rounded-lg">
          ...
          {groupedJobs.length > 0 ? (
            groupedJobs.map((jobGroup, i) => {
              // ambil satu job asli untuk detail, lalu gabungkan location menjadi string
              const repJob = allFilteredJobs.find(
                (j) => j.title === jobGroup.title
              )!;
              const jobForDetail: Job = {
                ...repJob,
                // override location dengan daftar lokasi gabungan
                location: jobGroup.locations,
              };

              return (
                <div
                  key={jobGroup.title}
                  onClick={() => openDetail(jobForDetail)}
                  className={`flex cursor-pointer font-bold text-xs md:text-base hover:bg-[#1E1E1E66] ${
                    i % 2 === 1 ? "bg-[#1E1E1E1A]" : ""
                  }`}
                >
